...
#         val_files_set = set(map(lambda a: os.path.normpath(os.path.join(root, a)), f.read().rstrip("\n").split("\n")))
    
#     with open(test_file, "r") as f:
#         test_files_set = set(map(lambda a: os.path.normpath(os.path.join(root, a)), f.read().rstrip("\n").split("\n"))) 
...
def get_train_val_test_split(root: str, val_file: str, test_file: str):
    ...
    def read_file_list(file_path):
        with open(file_path, "r") as f:
            return set(map(lambda a: os.path.normpath(os.path.join(root, a.strip())), f.readlines()))
    
    val_files_set = read_file_list(val_file)
    test_files_set = read_file_list(test_file)
    
    assert len(val_files_set.intersection(test_files_set)) == 0, "Sanity check: No files should be common between val and test."
    
    train_files_set = all_files_set - val_files_set - test_files_set
    
    train_list = list(train_files_set)
...
class GoogleSpeechDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.cache:
            ...
            x, _ = librosa.load(self.data_list[idx], sr=self.audio_settings["sr"])  # Ensure sr is specified correctly
